...
import { settings, runs, tweets, bookmarks, users } from "./drizzle/schema";
  ...
  try {
    ...
    if (backupData.tables.users && backupData.tables.users.length > 0) {
      ...
      for (const user of backupData.tables.users) {
        const converted = convertDates(user, ['createdAt', 'updatedAt', 'lastSignedIn']);
        await db.insert(users).values(converted).onDuplicateKeyUpdate({ set: converted });
      }
    ...
    if (backupData.tables.settings && backupData.tables.settings.length > 0) {
      ...
      for (const setting of backupData.tables.settings) {
        const converted = convertDates(setting, ['createdAt', 'updatedAt']);
        await db.insert(settings).values(converted).onDuplicateKeyUpdate({ set: converted });
      }
    ...
    if (backupData.tables.runs && backupData.tables.runs.length > 0) {
      ...
      for (const run of backupData.tables.runs) {
        const converted = convertDates(run, ['startedAt', 'completedAt', 'createdAt']);
        await db.insert(runs).values(converted).onDuplicateKeyUpdate({ set: converted });
      }
    ...
    if (backupData.tables.tweets && backupData.tables.tweets.length > 0) {
      ...
      for (let i = 0; i < backupData.tables.tweets.length; i += batchSize) {
        ...
        for (const tweet of batch) {
          const converted = convertDates(tweet, ['createdAt', 'fetchedAt']);
          await db.insert(tweets).values(converted).onDuplicateKeyUpdate({ set: converted });
        }
    ...
    if (backupData.tables.bookmarks && backupData.tables.bookmarks.length > 0) {
      ...
      for (const bookmark of backupData.tables.bookmarks) {
        const converted = convertDates(bookmark, ['createdAt']);
        await db.insert(bookmarks).values(converted).onDuplicateKeyUpdate({ set: converted });
      }
